...
import {
  commandTeamRun, completeTeamTask, createTeamRun, getDispatchReceipts, getTaskWorktrees, getTeamRun, getTeamRunEvents, mergeTaskWorktree, releaseTaskWorktree, reviewTeamTask,
  type DispatchReceipt, type ManagedSession, type TaskWorktree, type TeamRunEvent, type TeamRunSnapshot, type TeamRunSummary, type TeamTaskInput,
} from "../../app/api";
...
export default function TeamRunPanel({ sessions, csrfToken, runs, onRefresh }: {
  ...
  async function refresh() {
    if (!snapshot) return;
    const [next, nextEvents, nextDispatches, nextWorktrees] = await Promise.all([
      getTeamRun(snapshot.run.run_id), getTeamRunEvents(snapshot.run.run_id), getDispatchReceipts(snapshot.run.run_id), getTaskWorktrees(snapshot.run.run_id),
    ]);
    setSnapshot(next); setEvents(nextEvents); setDispatches(nextDispatches); setWorktrees(nextWorktrees);
  }
  ...
  async function merge(worktree: TaskWorktree) {
    if (!snapshot || !window.confirm(`合并 ${worktree.branch} 到项目当前分支？合并前会检查主目录是否干净。`)) return;
    setBusy(true);
    try { await mergeTaskWorktree(snapshot.run.run_id, worktree.task_id, worktree.attempt, csrfToken); await refresh(); setNotice(`已合并 ${worktree.branch}。可在确认后释放隔离目录。`); }
    catch { setNotice("无法合并：主项目可能有未提交改动、发生冲突，或任务尚未验收。"); }
    finally { setBusy(false); }
  }
  async function release(worktree: TaskWorktree) {
    if (!snapshot || !window.confirm(`释放 ${worktree.branch} 的 tmux CLI 与隔离目录？仅已合并成果可释放。`)) return;
    setBusy(true);
    try { await releaseTaskWorktree(snapshot.run.run_id, worktree.task_id, worktree.attempt, csrfToken); await refresh(); setNotice(`已释放 ${worktree.branch} 的隔离 CLI 和工作目录。`); }
    catch { setNotice("无法释放：请先合并已验收成果，或检查 worktree 状态。"); }
    finally { setBusy(false); }
  }
  ...
  return <section className="teamrun-panel" id="teamrun">
    ...
    {!snapshot ? <form className="teamrun-form" onSubmit={create}>
      ...
      <section className="teamrun-audit" aria-label="协作事件审计"><header><div><span>EVENT AUDIT / 可审计协作过程</span><h3>运行事件与 tmux 投递回执</h3></div><button className="secondary-action" onClick={() => void refresh()}>刷新</button></header><div className="dispatch-receipts">{dispatches.map((dispatch) => <div key={dispatch.command_id} className={`dispatch-receipt is-${dispatch.state}`}><strong>{dispatch.task_id} · #{dispatch.attempt}</strong><span>{dispatch.state === "tmux_written" ? "已写入 tmux" : dispatch.state === "pending" ? "等待投递" : "投递不确定，需人工确认"}</span>{dispatch.last_error && <small>{dispatch.last_error}</small>}</div>)}{!dispatches.length && <small>尚未生成 tmux 投递记录。</small>}</div>{worktrees.length > 0 && <div className="worktree-ledger"><strong>隔离工作目录</strong>{worktrees.map((worktree) => <div key={`${worktree.task_id}-${worktree.attempt}`}><span>{worktree.task_id} · #{worktree.attempt}</span><code>{worktree.branch}</code><small>{worktree.state === "active" ? "独立执行中" : "已释放"}</small>{snapshot.tasks.find((task) => task.task_id === worktree.task_id)?.state === "accepted" && worktree.state === "active" && <><button className="secondary-action" disabled={busy} onClick={() => void merge(worktree)}>合并已验收成果</button><button className="text-danger" disabled={busy} onClick={() => void release(worktree)}>释放目录</button></>}</div>)}</div>}<ol className="event-timeline">{events.slice().reverse().map((item) => <li key={item.event_id}><time>{new Date(item.occurred_at).toLocaleTimeString()}</time><strong>{labelEvent(item.event_type)}</strong><span>{item.aggregate_id}</span></li>)}{!events.length && <li>尚无事件；开始 TeamRun 后会显示可核验的协作过程。</li>}</ol></section>
