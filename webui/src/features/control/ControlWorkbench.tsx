...
import {
  ...
  inspectProject, launchTeamRun, probeProvider, releaseManagedSession, scanProviders,
  stopManagedSession, updateProject,
...
export default function ControlWorkbench({ csrfToken, providers, projects, managedSessions, sessions, onRefresh, teamRuns }: Props) {
  ...
  async function stopSession(item: ManagedSession) { if (!window.confirm(`关闭“${item.name}”的 tmux？绑定和历史会保留，下一条 IM 消息会自动恢复。`)) return; setBusy(`stop-${item.id}`); try { await stopManagedSession(item.id, csrfToken); setNotice("tmux 已关闭；下一条 IM 消息到达时自动恢复。"); await onRefresh(); } catch { setNotice("关闭 tmux 失败；活动 TeamRun 需要先停止。"); } finally { setBusy(null); } }
