  ...
  return <>
    <section className="workbench" aria-label="项目启动与本机控制">
      ...
      <section className="wizard" aria-label="创建项目向导">
        ...
        {step === 4 && <div className="wizard-stage launch-review"><h3>确认 tmux 启动计划</h3><p>将创建的 CLI 都在 <code>{projectPath}</code> 内运行。默认启用 CLI 当前权限模式；模型由每个 CLI 自己的原生 picker 决定。</p><ul>{currentRecipe.roles.map((role) => <li key={role.id}><strong>{role.label}</strong><span>{llmProviders.find((provider) => provider.id === (roleProviders[role.id] || providerId))?.binary_name || "未选择"}</span><small>{role.schedulerRole ? `可加入 TeamRun：${role.schedulerRole}` : "独立设计会话；第一版由统筹手动交接"}</small></li>)}</ul><label><span>协作目标（可选）</span><textarea value={runGoal} onChange={(event) => setRunGoal(event.target.value)} placeholder="创建完成后，可在下方 TeamRun 协作台粘贴此目标并启动确定性三角色调度。" /></label><small>当前 TeamRun 已支持统筹、实施、审查三角色。UI 设计者会作为独立 tmux CLI 保留上下文，自动设计→实施交接将在调度器扩展时加入。</small><div className="wizard-actions"><button className="secondary-action" onClick={() => setStep(3)}>返回</button><button className="primary-action" disabled={busy !== null} onClick={() => void launchRecipe()}>{busy === "launch" ? "正在创建 tmux 会话…" : `确认创建 ${currentRecipe.roles.length} 个 CLI`}</button></div></div>}
      </section>
      ...
      <div className="workbench-columns">
        ...
        <article className="workbench-unit"><span className="unit-number">TMUX WINDOWS</span><h3>查看与操作终端</h3><p>所有受管会话默认以只读方式打开；只有手动接管后才可发送输入。</p><div className="terminal-list">{managedSessions.map((item) => <div className="managed-row" key={item.id}><button type="button" className="session-row" onClick={() => setTerminalSession(item)}><span>{item.name}</span><code>{item.tmux_target}</code><strong>查看终端 / 原生模型选择</strong></button><button className="text-danger" onClick={() => void releaseSession(item)} disabled={busy !== null}>释放管理</button></div>)}</div>{unmanaged.length > 0 && <div className="orphan-list"><h4>现有 tmux pane</h4>{unmanaged.map((item) => <div key={item.target} className="orphan-row"><div><strong>{item.session_name}</strong><code>{item.target} · {item.cwd}</code></div><select defaultValue="" aria-label={`${item.target} 所属项目`} onChange={(e) => { const [projectId, provider] = e.target.value.split(":"); if (projectId && provider) void adopt(item, projectId, provider); }} disabled={busy !== null}><option value="">纳入项目并打开…</option>{projects.flatMap((project) => llmProviders.map((provider) => <option key={`${project.id}-${provider.id}`} value={`${project.id}:${provider.id}`}>{project.name} · {provider.binary_name}</option>))}</select></div>)}</div>}</article>
