...
- OMP `/new` 先在内存创建会话，首条 assistant 回复前可能没有新 JSONL。即时成功以当前 TUI 的 `New session started` 状态行为准；前导成功图标兼容 OMP 实际使用过的 `✓` 与 `✔`，不能把单一 Unicode 字形当成协议。handoff 保持 armed，待新 transcript 落盘后再持久化 identity。
