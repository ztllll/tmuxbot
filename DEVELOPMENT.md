...
- OMP 的 IM 状态栏使用产品自有、跨版本稳定的语义格式，不复制某一版 TUI 的框线、图标或字段顺序。`OmpBackend.parse_terminal_status()` 只从 pane 最后两条非空行构成的当前 footer 中提取 model/provider、context percent/limit 与 thinking effort；支持旧 `╭── π` footer 和新版 compact `+-- … --+` footer，未知布局 fail closed。exact handoff/pin 指向的 session JSONL `stat().st_size` 由 enrichment seam 补入。IM 固定渲染为 `模型 … · 上下文 … · 思考 … · JSON …`，JSONL metadata 只补 TUI 缺失字段，不覆盖已解析的显示名。
