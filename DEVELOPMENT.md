...
- Claude/Codex 的 `paste-buffer -p` 前先等 TUI idle。OMP 是显式例外：普通文字/附件在 Working 时可提交到原生 steering queue（`accepts_input_while_busy=True`）；`/new`、`/compact`、`/clear`、`/fresh` 及所有 picker/control command 必须要求 IDLE，busy 时立即回复未执行，不能排队等待或进入 steering。OMP 17.3.3 的 compact ASCII 与 Unicode footer 状态行下方同时是活动 composer；tmuxbot 分别读取 `| … |`/`+- … -+` 和 `│ … │`/`╰─ … ─╯` 输入行，并忽略 footer 后的补全 overlay。附件 `@path` 首个 Enter 若只关闭文件补全、草稿仍在，则按既有有界确认状态机再次 Enter，直到 composer 清空/进入 provider 工作态或达到重试上限。
...
OMP `/new` 先在内存创建会话，首条 assistant 回复前可能没有新 JSONL。即时成功以当前 TUI 的 `New session started` 状态行为准；前导成功图标兼容 OMP 实际使用过的 `✓` 与 `✔`，不能把单一 Unicode 字形当成协议。handoff 保持 armed；新 OMP 会话与从旧 Pi 精确恢复后继续使用 `.pi` 根目录的迁移会话都允许 IM 注入首条消息，tailer 待 transcript 落盘后再持久化 identity。若 bridge 在 `/new` 与首条回复之间重启，sidecar 指向的新 transcript 与持久化旧 pin 不同，tailer 必须从 offset 0 读取该新文件，绝不能将首条回复当 bootstrap 历史跳过。
- OMP 的 IM 状态栏使用产品自有、跨版本稳定的会话决策格式，不复制某一版 TUI 的框线、图标或字段顺序。`OmpBackend.parse_terminal_status()` 只从 pane 最后两条非空行构成的当前 footer 中提取 model/provider、context percent/limit、thinking effort、auto-compact 与可见的 `tok`；支持旧 `╭── π` footer 和新版 compact `+-- … --+` footer，未知布局 fail closed。IM 按决策优先级固定渲染为 `上下文 … (已用/上限，余 …) · 会话累计 … tok · 缓存命中 … · 自动压缩 · 模型 … · 思考 …`。本机 OMP 17.3.3 的 `token_total` 定义为 `input + output + cacheWrite + orchestrationInput + orchestrationOutput`，不含 `cacheRead`；footer 隐藏时从当前 branch JSONL usage 同口径补齐。exact session JSONL 文件大小仅保留为内部运行元数据，不进入默认状态栏，也不作为 token 占用指标。
...
- OMP 对成功的结构化文件变更额外产生 tool progress：`edit` 只消费 `toolResult.details.perFileResults[].diff`，`write` 只消费同一 `toolCallId` 已缓存的本地文件内容；均在成功结果落盘后才显示，不展示未执行的提案。IM 使用转义后的 `<pre>` 代码块并限制文件数、行数和字符数；`xd://`/`local://` 等内部 URI、`.env`、密钥、凭证、token/password 命名文件与私钥格式 fail closed，不投影内容。
