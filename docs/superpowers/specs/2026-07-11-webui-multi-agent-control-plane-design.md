...
7. `RunEvent` is the only cross-channel runtime state input. / WebUI、Telegram、飞书和通知只消费统一的 `RunEvent`，不各自推断状态。
8. Project coordination configuration is session-scoped. / 协调工具按项目与 TeamRun 隔离，不覆盖用户全局 CLI、MCP 或指令文件。
...
tmuxbot-web              TelegramFrontend       FeishuFrontend
    ...
    └───────────────────────────┴───────────────────────┘
       ...
       Session Registry   TeamRun Scheduler   Event/Artifact Store
              ...
              └─────────────────┼──────────────────┘
                                │
                    Session-scoped Coordinator MCP
                                │
...
- **Lease**：共享目录写锁或集成锁。
- **FileClaim**：任务计划修改的文件或 glob 范围，只用于提前发现冲突，不替代最终 diff 校验。
- **WorkspaceHook**：有超时、输出限制和审计记录的 worktree 初始化或销毁动作。
- **RunEvent**：追加式审计事件。
...
9. Generate a TeamRun-scoped Coordinator MCP configuration without modifying global MCP settings.
10. Apply an allowlisted `.worktreeinclude` and bounded workspace setup hook when worktree mode is selected.
11. Validate hooks or Coordinator MCP only after confirmation.
12. Persist all roles, targets, identities, capabilities, ports, and owned process identifiers.
...
- Planned file/glob claims are checked before assignment; overlapping claims require serialization or operator approval.
- Each writer produces a commit and validation evidence.
- Actual changed paths are compared with claims after execution; undeclared overlap blocks integration.
...
- `.worktreeinclude` copies only explicitly required ignored files; secrets and the complete ignored tree are never copied implicitly.
- Setup and teardown hooks are allowlisted, time-bounded, output-bounded, and followed by owned-port and owned-process cleanup.
...
The Web service creates a separate endpoint and short-lived credential for each TeamRun. Provider launch commands receive the endpoint through run-scoped environment or generated local configuration. tmuxbot never rewrites user-global Codex, Claude, MCP, `CLAUDE.md`, or `AGENTS.md` settings.

Web 服务为每个 TeamRun 建立独立端点和短期凭据，通过运行级环境变量或生成的局部配置交给 CLI。tmuxbot 不覆盖用户全局 Codex、Claude、MCP、`CLAUDE.md` 或 `AGENTS.md` 配置。
...
3. Durable project knowledge and operator-owned rules in `CLAUDE.md`, `AGENTS.md`, specs, and conventions.

Provider-private transcripts remain private by default. TeamRun artifacts are deliberately published and immutable after acceptance. Durable project knowledge is changed only through a normal reviewed repository change, never through a shared writable wiki controlled directly by agents.
...
After service restart, it reconciles SQLite with tmux sessions, pane commands, worktrees, owned ports/processes, and provider transcripts. Orphaned sessions are surfaced for attach/adopt/archive/ignore and are never killed automatically. Adoption records the previous target and observed provider identity before the session becomes schedulable.
...
- Control takeover requires a short-lived terminal ticket, records operator identity, target, start/end time, and blocks scheduler keystroke injection while active.
...
Desktop uses the complete Command Center. Mobile is a deliberately reduced remote-control surface: one selected terminal, Agent switcher, run summary, approvals, pause/resume/stop, and artifact download. It does not compress the full desktop graph into a small viewport.
...
- All channel notifications are projections of persisted `RunEvent` records; channel adapters do not maintain an independent task state machine.
...
1. Domain models, SQLite, authentication, append-only `RunEvent`, and read-only tmux inventory with orphan classification.
...
7. Worktree parallel mode, `.worktreeinclude`, bounded workspace hooks, file claims plus actual-diff validation, merge queue, and integration lease.
...
- `RunEvent` projection contract tests proving WebUI, Telegram, and Feishu receive the same persisted state transition.
...
- Orphan E2E: an unmanaged tmux target is discovered and can be ignored, archived, or adopted without being killed.
- Worktree E2E: include allowlist, bounded hooks, file-claim preflight, actual-diff conflict detection, and owned-process cleanup.
...
- Session-scoped Coordinator MCP; no global CLI/MCP/instruction-file mutation.
- Unified persisted `RunEvent` projections for WebUI, Telegram, Feishu, and notifications.
- File claims are advisory preflight controls and must be verified against actual Git diffs.
- Desktop Command Center and simplified mobile remote-control surface are separate responsive compositions.
