...
- transcripts are OMP JSONL v3 under `~/.omp/agent/sessions/...`; exact legacy sessions resumed from `~/.pi/agent/sessions/...` remain valid during migration, including the provider-authored pending path published by `/new` before the file exists. Discovery does not scan either tree. `omp-session-handoffs/` records exact target, canonical cwd, session ID, transcript path, and live pane process ID; `omp-session-health/` records the same session identity and health state. Before a brand-new session creates its JSONL, only an official OMP or legacy Pi sessions-root path with matching session ID and process in the exact pane is accepted; the tailer waits without treating the absent pending file as an error. Once the file exists, header/cwd/session validation is mandatory. A live OMP process without a valid managed identity sidecar is not safe for IM injection;
