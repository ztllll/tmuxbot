"""Static SSH handoff notices for OMP interactions that require a real TUI."""
...
def omp_ssh_interaction_notice(
    binding: object,
    *,
    interaction_label: str = "交互界面",
    confirmed: bool = True,
) -> str:
    ...
    status = (
        ...
        if confirmed
        else f"· 如需{html.escape(interaction_label)}，"
    )
    return (
        "⚠️ <b>OMP 需要交互式操作</b>\n"
        f"{status}tmuxbot 不会通过 IM 模拟按键或代替选择。\n"
        "· 请 SSH 登录对应主机后执行：\n"
