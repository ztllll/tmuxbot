...
- 修复 Telegram/飞书向 Pi 注入 `/new` 后错误提示“未确认完成”：Pi 会先在内存切换并显示 `✓ New session started` 或 `✔ New session started`，直到新会话首条 assistant 回复才创建 JSONL。tmuxbot 现在兼容两种实际 TUI marker，确认即时成功并保留 session handoff，待首条回复落盘后再持久化新 identity。
