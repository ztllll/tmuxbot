...
- 适配最新 Pi TUI extension 组合：同时解析原生 footer 与 `pi-statusline` powerline，恢复 provider/model/thinking/cwd/Git/context/token/cache/cost/JSONL 状态；composer 提交确认也识别自定义 statusline，不会因 Todo overlay 位于编辑器上方而退回单次 Enter。
