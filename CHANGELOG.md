...
- Admin DM 配置现在拒绝直接使用 Unix 用户根目录作为 `TMUXBOT_ADMIN_CWD`，强制使用独立私有 workspace，避免受管 `AGENTS.md` / `CLAUDE.md` 被普通项目沿目录树继承；配置测试显式使用临时 Admin cwd，不再覆盖真实 workspace 的 manifest。
...
- 修复人工停止 Pi 长等待/工具执行后的误报：Pi 某些版本会把显式取消持久化为 `stopReason=error` 与 `This operation was aborted`。backend 兼容回推、受管 session-health extension 和十分钟 terminal-health audit 现在都将已知 cancellation 文案视为用户操作并静默，不会伪装成 provider 终态故障；正常长轮询仍因活跃工具证据保持静默。
...
- 完整落地结果优先 IM 消息生命周期：新增通道无关 `TurnProjection`，将一个 provider Turn 内的工具调用、计划更新和普通生命周期事件压缩为单一可刷新过程卡；`compact` 默认延迟 4 秒创建并以 2 秒节流、内容 hash 去重和最近三项确定性摘要原地更新，短任务只发结果。`TEXT_DELTA` / live text 仅进入内存 ResultDraft，`FINAL_TEXT` 统一经过 `ReplyEnvelope`、附件、分页和 footer 独立发布一次；重复 final 被抑制。interaction、waiting 与终态 provider error 使用独立注意消息，过程卡 edit/PATCH 失败安全重建且不阻断结果。新增 `result_only` / `compact` / `verbose` 配置和不含正文的 `im-delivery-audit.json` 计数；Telegram 与飞书复用同一语义，Pi 自动压缩专用状态卡保持原流程兼容。
...
- 新增可配置的严格 Boss Admin DM route（初版默认 cwd 为运行账户 `Path.home()`，现已迁移到独立 XDG Admin workspace，见本版上方 Added），CLI 可选 Pi/Claude/Codex；新增 `docs/admin-dm-operations.md`，记录自然语言开通/绑定模板、确定性操作顺序、安全约束和真实 IM 验收清单。
...
- 修复 Pi Working 状态下飞书/TG 新消息等待 300 秒后 `TmuxBusyTimeout`：Pi 官方 TUI 支持 Enter 将 streaming 期间的输入加入 steering queue，现通过 provider capability 立即提交文字和附件，不再套用 Claude/Codex 的 pre-paste idle 门禁；busy 路径以 composer 清空/变化确认已入队，不能仅凭仍在 Working 假确认。Claude/Codex 继续在 paste 后、每次 Enter 前要求稳定 idle 0.5s，防止竞态和重复提交。
