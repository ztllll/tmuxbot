...
- **本机 tmux Terminal Wall**：`tmuxbot serve --open` 展示宿主机全部 tmux window；xterm.js 经 tmux Control Mode 镜像真实 pane，并保留颜色、快捷键、粘贴和交互。卡片可任意拖动、八向缩放、置顶、横/纵/网格排列与全屏；单端使用时卡片缩放会像 SSH/Tabby 一样重排真实 TUI，关闭 WebUI 后下一次 SSH/Tabby attach 接管尺寸；浏览器布局存入 localStorage。
