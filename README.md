...
- **中文控制面板**:`/panel` 或 `/settings` 主动打开轻量面板，可切换群聊 @ 策略、执行 `/status` `/screen` `/new` `/compact` `/resume` `/esc` `/cc`，并通过当前 tmux CLI 的原生 `/model` 选择器切换模型；Claude 模型卡额外提供“仅本会话”，避免修改未来新会话默认模型
